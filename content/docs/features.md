...
| **Search (nav)** | With a nav from `pages.yml`, a **search** field appears at the top of the sidebar. The index is built when the field is **focused** (refetched on each focus): every listed `path` is fetched, **nav titles** + **frontmatter `title`** + **Markdown body** (code blocks stripped) are searchable. Multi-word queries require **all** words to match. Press **`/`** to focus search when not typing in a field. |
