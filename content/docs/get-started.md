...
1. **Clone and run a static server** (ES modules and `fetch` will not work from `file://` the way you need):
   ...
   npm run dev
   ```
2. **Open** `http://127.0.0.1:3456/` (or the port in the log). The default view is set by `default_path` in `pages.yml` (here: [Framework home](#docs/index)) unless the URL has a hash.
3. **Change content** under [`content/`](https://github.com/eSlider/md-frontend-framework/tree/main/content); refresh. Deep links: `#docs/philosophy`, `#example`, etc. (no `.md` in the bar).
